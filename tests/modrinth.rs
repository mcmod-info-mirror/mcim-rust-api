...
#[actix_web::test]
async fn test_modrinth_projects() {
    let app = init_service(create_test_app().await).await;
    let ids_json = serde_json::to_string(PROJECT_IDS).unwrap();
    let req = TestRequest::get()
        .uri(&format!("/modrinth/v2/projects?ids={}", urlencoding::encode(&ids_json)))
        .to_request();
    let resp = app.call(req).await.unwrap();
...
#[actix_web::test]
async fn test_modrinth_project_versions() {
    let app = init_service(create_test_app().await).await;
...
async fn test_modrinth_versions() {
    ...
    let req = TestRequest::get()
        .uri(&format!("/modrinth/v2/versions?ids={}", urlencoding::encode(&ids_json)))
        .to_request();
    let resp = app.call(req).await.unwrap();
...
async fn test_modrinth_version_file_sha1() {
    ...
    for sha1_hash in SHA1_SAMPLE {
        let req = TestRequest::get()
            .uri(&format!("/modrinth/v2/version_file/{}?algorithm=sha1", sha1_hash))
            .to_request();
        let resp = app.call(req).await.unwrap();
...
async fn test_modrinth_version_file_sha512() {
    ...
    for sha512_hash in SHA512_SAMPLE {
        let req = TestRequest::get()
            .uri(&format!("/modrinth/v2/version_file/{}?algorithm=sha512", sha512_hash))
            .to_request();
        let resp = app.call(req).await.unwrap();
...
async fn test_modrinth_version_file_sha1_update() {
    ...
    for sha1_hash in SHA1_SAMPLE {
        let req = TestRequest::post()
            .uri(&format!("/modrinth/v2/version_file/{}/update?algorithm=sha1", sha1_hash))
            .set_json(&json!({
                "loaders": ["fabric"],
...
async fn test_modrinth_version_file_sha512_update() {
    ...
    for sample in get_update_samples() {
        ...
        let req = TestRequest::post()
            .uri(&format!("/modrinth/v2/version_file/{}/update?algorithm=sha512", sample.hash))
            .set_json(&json!({
